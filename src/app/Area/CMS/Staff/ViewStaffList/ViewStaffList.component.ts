...
@Component({
  ...
  styleUrls: ['./ViewStaffList.component.css'],
  standalone: false,
})
export class ViewStaffListComponent implements OnInit {
  // #region parameter declaration
  editid: number = 0;
  stafflist: IStaffDetailSearchResponse[] = [];
  public staffsearch: IStaffSearchRequest = {
    columnName: '',
    searchParam: '',
  };
  public staffcount: IStaffCountResponse = {
    totalStaff: 0,
    male: 0,
    female: 1,
    teaching: 0,
  ...
  ngOnInit() {
    ...
    if (this.frameworkService.CSSE('filter')) {
      ...
      const mapping: { [key: string]: keyof IStaffSearch } = {
        Name: 'name',
  ...
  GetStaffList() {
    this.staffService.getStaffList(this.staffsearch).subscribe({
      next: (Response) => {
        if (Response.data != null) {
  ...
  onDepartmentNameChanges(options: { value: string; text: string }) {
    this.onFieldChange('departmentCode', 'DepartmentCode', options);
  }
  ...
  onEditClick(value: any): void {
    ...
    this.router.navigate(['CMS/ViewStaff']);
  }
  ngAfterViewInit() {
