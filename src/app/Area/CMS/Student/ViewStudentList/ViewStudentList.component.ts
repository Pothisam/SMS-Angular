...
export class ViewStudentListComponent implements OnInit {
  ...
  public tableSettings: ITableSettings = {
    ...
    columns: [
      ...
      { title: 'Batch', data: 'batch', short: true, width: 10 },
  ...
  ngAfterViewInit() {
    if (this.fws.CSSE('filter')) {
      ...
      if (this.searchrequest.departmentCode !== null) {
        this.triggercourseApi = true;
      }
      if (this.searchrequest.courseCode !== null) {
  ...
  clearrequest() {
    this.searchrequest.departmentCode = '';
    this.searchrequest.courseCode = '';
    this.searchrequest.batch = '';
  }
  onNameChange() {
  ...
  onEditClick(value: any): void {
    ...
    this.router.navigate(['CMS/ViewStudent']);
  }
  // #endregion
}
